...
while True:
    ...
    if soldOut == "SOLD OUT":
        with open('config.yaml', "r") as file:
            try:
                databaseConfig = yaml.safe_load(file)   
                n_request = (databaseConfig["request_number"])
            except yaml.YAMLError as exc:
                print(exc)
        count+=1
        now = datetime.now()
        print(sito + "\n" + "NUM CONTROLLO: " + str(count) + "\n" + "Non è disponibile" + " " + "\n" "Data controllo: " + str(now) + "\n" + "Richieste: " + str(count) + "/" + str(n_request) + "\n")
        if count >= n_request + 1:
            count = count - 1
            print("Termino il programma. Ho eseguito " + str(count) + " richieste." )
